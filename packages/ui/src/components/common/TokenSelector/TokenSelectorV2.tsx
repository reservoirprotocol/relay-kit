...
const TokenSelectorV2: FC<TokenSelectorProps> = ({
  token,
  restrictedToken,
  trigger,
  ...
  // Filter dune token balances based on restricted tokens and configured chains
  const duneTokenBalances = useMemo(() => {
  ...
  }, [duneTokens?.balances, filteredRestrictedTokenList, configuredChainIds])

  // Update token list query to include suggested tokens
  const suggestedTokenQuery = useMemo(() => {
    if (filteredRestrictedTokenList && filteredRestrictedTokenList.length > 0) {
      return filteredRestrictedTokenList.map(
        (token) => `${token.chainId}:${token.address}`
      )
    } else if (duneTokenBalances) {
      return duneTokenBalances.map(
        (balance) => `${balance.chain_id}:${balance.address}`
      )
    }
    return undefined
  }, [filteredRestrictedTokenList, duneTokenBalances])

  // Get user's tokens with suggested tokens
...
export default TokenSelectorV2
