      ...
      >
        ...
        >
          {chains
            ...
            .map((chain) => {
              const currencies = [
                { ...chain?.currency, address: zeroAddress },
                ...(chain?.erc20Currencies || [])
              ]
              const chainCurencyAddress = currencies?.find(
                (chainCurrency) => chainCurrency?.id === currency?.id
              )?.address
              const balance =
                address &&
                chainCurencyAddress &&
                tokenBalances?.[`${chain.id}:${chainCurencyAddress}`]
                  ? tokenBalances?.[`${chain.id}:${chainCurencyAddress}`]
                  : null
              const compactBalance = Boolean(
                balance?.amount &&
                  balance?.decimals &&
                  balance?.amount.toString().length - balance?.decimals > 4
              )
